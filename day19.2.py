...
def dist_r3(a, b):
    return int(sqrt((a[X] - b[X]) ** 2 + (a[Y] - b[Y]) ** 2 + (a[Z] - b[Z]) ** 2))
...
def obtain_max_manhattan_distance(offset_by_scanner):
    max_dist = 0
    for i in xrange(len(offset_by_scanner) - 1):
        offset1 = offset_by_scanner[i]
        for j in xrange(i + 1, len(offset_by_scanner)):
            offset2 = offset_by_scanner[j]
            dist = dist_manhattan(offset1, offset2)
...
def process(lines):
    ...
    return obtain_max_manhattan_distance(offset_by_scanner)
